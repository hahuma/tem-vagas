<template>
  <div class="page-content">
    <div class="container-fluid">
      <NavigatorList v-model="search" title="Lista de Empresas" @action="() => router.push('/admin/empresa')" />

      <NoRegisters v-if="companies.length == 0 && !loading" text="Nenhuma empresa encontrada" />

      <div v-else class="h-100 p-4 overflow-overlay page-app">
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-2 g-4">
          <div v-for="item in companies" :key="item.uuid" class="col">
...
const advancedSearch = async (nextPage = false) => {
  ...
  loading.value = false;
  const { data } = await axios.get("/companies/list", { params: criteria.value });
...
const showDetails = (id) => {
  router.push({ path: "/admin/empresa/" + id });
};
