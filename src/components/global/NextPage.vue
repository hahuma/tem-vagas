<template>
  <div class="row d-flex justify-content-center align-items-center py-4 px-2">
    <div :class="basicStyle" v-bind="$attrs">
      Ver mais resultados
      <svg class="ms-2" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M13.6498 2.35C12.1998 0.9 10.2098 0 7.99977 0C3.57977 0 0.00976562 3.58 0.00976562 8C0.00976562 12.42 3.57977 16 7.99977 16C11.7298 16 14.8398 13.45 15.7298 10H13.6498C12.8298 12.33 10.6098 14 7.99977 14C4.68977 14 1.99977 11.31 1.99977 8C1.99977 4.69 4.68977 2 7.99977 2C9.65977 2 11.1398 2.69 12.2198 3.78L8.99977 7H15.9998V0L13.6498 2.35Z"
          fill="#FFF7F2"
        />
      </svg>
    </div>
  </div>
</template>

<script setup>
const basicStyle = 'btn col-sm-2 col-md-5 col-lg-3 btn-primary gradient-orange btn btn-primary fs-5 py-1 px-4 border-0'
</script>
